const translation = {
  'About us': 'Chúng tôi',
  Token: 'Token',
  Ecosystem: 'Hệ sinh thái',
  Roadmap: 'Lộ trình',
  Contract: 'Hợp đồng',
  Collection: 'Bộ sưu tập',
  'Multi-Industry Blockchain Technology & Payment Platform (Super DApp)':
    'Nền tảng Công nghệ & Thanh toán Blockchain Đa ngành (Super DApp)',
  'JOIN NOW': 'THAM GIA NGAY',
  'WHITE PAGE': 'TRANG TRẮNG',
  'Introduction to': 'Giới thiệu về',
  'Amidst the global shift toward a digital economy, along with the growingtrend of green and sustainable development. AZChain Platform was created to become a specialized Blockchain platform, pioneering in addressing emerging demands.':
    'Trước làn sóng chuyển đổi mạnh mẽ của nền kinh tế toàn cầu sang các giải pháp số, cùng với xu hướng phát triển kinh tế xanh và bền vững, AZChain Platform ra đời, trở thành một nền tảng blockchain chuyên biệt, tiên phong trong việc đáp ứng những nhu cầu đổi mới.',
  'AZChain Platform offers secure and transparent Blockchain solutions for managing data and authenticating real-world assets, enabling fast andhighly secure transactions.':
    'AZChain Platform mang đến những giải pháp Blockchain an toàn, minh bạch trong việc quản lý Dữ liệu và mã hóa Tài sản thực, khả năng thực hiện giao dịch nhanh chóng và bảo mật cao.',
  'With a focus on meeting diverse needs in Finance, Commerce, and Governance, that also contributes to the advancement of the digital and green economy.':
    'Nhằm đáp ứng nhu cầu đa dạng trong lĩnh vực Tài chính, Thương mại và Quản trị, cũng như góp phần thúc đẩy cả nền Kinh tế số và nền Kinh tế xanh.',
  Transparency: 'Minh bạch',
  'Ensure all transactions and data are verifiable and trustworthy.':
    'Đảm bảo mọi giao dịch và dữ liệu đều có thể được kiểm tra tin cậy.',
  Security: 'An toàn',
  'Enhance protection at every level to safeguard user interests.':
    'Tăng cường bảo mật ở mọi cấp độ để bảo vệ quyền lợi của người dùng.',
  Fairness: 'Công bằng',
  'Bring blockchain technology closer to everyone, enabling access without complex barriers.':
    'Ứng dụng Blockchain đến gần với mọi người, giúp tiếp cận công nghệ mà không gặp rào cản phức tạp.',
  Efficiency: 'Hiệu quả',
  'Improve operational performance for both businesses and individuals with flexible and cost-effective blockchain solutions.':
    'Nâng cao hiệu quả hoạt động cho cả Doanh nghiệp và Cá nhân thông qua giải pháp Blockchain linh hoạt và tiết kiệm chi phí.',
  Speed: 'Tốc độ',
  'Fast transaction and data processing for time-saving & improved user experience.':
    'Tối ưu tốc độ xử lý giao dịch và truyền tải dữ liệu nhanh, tiết kiệm thời gian, nâng cao trải nghiệm thực tế.',
  'Platform Technology & Payment Blockchain Multi-industry (Super DApp)':
    'Nền tảng Công nghệ & Thanh toán Blockchain Đa ngành (Super DApp)',

  // AZGPay
  'MORDEN BLOCKCHAIN PAYMENT PLATFORM':
    'NỀN TẢNG THANH TOÁN BLOCKCHAIN HIỆN ĐẠI',
  'Supports Cryptocurrency & Fiat': 'Hỗ trợ Tiền mã hóa & Tiền Fiat',
  'Multi-Chain Digital Asset Management': 'Quản lý Tài sản số Đa chuỗi',
  'Crypto Visa/Mastercard': 'Thẻ Visa/Mastercard Crypto',
  'Staking, NFT, DeFi': 'Staking, NFT, DeFi tiện lợi',
  'Secured by Blockchain': 'Bảo mật với Blockchain',
  'User-friendly Interface, High Speed': 'Giao diện thân thiện, tốc độ nhanh',

  // Benefit
  'Key Advantages of the AZChain Platform': 'Lợi ích AZChain Platform mang đến',
  'AZChain Platform unlocks The Multi-Industry Blockchain Ecosystem, delivering real value to customers, investors, and the global community':
    'AZChain Platform mở ra Hệ sinh thái Blockchain Đa ngành, mang đến giá trị thiết thực cho khách hàng, nhà đầu tư và cộng đồng toàn cầu',
  'Multi-Industry Blockchain Platform (Super DApp)':
    'Nền tảng Blockchain Đa ngành (Super DApp)',
  '11+++ Money Streams': '11+++ Dòng tiền Thu nhập',
  'Profits for Investors & Platform Developers':
    'Lợi nhuận cho Nhà đầu tư & Nhà phát triển Nền tảng',
  'Franchise Startup on the Global Digital Platform (120 USDT)':
    'Doanh nghiệp nhượng quyền trên Nền tảng Số toàn cầu (120 USDT)',
  'Advancement Opportunity': 'Cơ hội phát triển',
  '6.000+ Diverse & Unique Products, with 5% to 40% Commissions & Bonuses':
    '6.000+ Sản phẩm đa dạng & Độc đáo, với 5% đến 40% Hoa hồng & Phần thưởng',
  'AZC ACADEMY: AI, Blockchain, Web3': 'AZC ACADEMY: AI, Blockchain, Web3',
  'Mission to Support Blockchain in Underdeveloped Countries':
    'Sứ Mệnh Hỗ Trợ Blockchain cho các Quốc gia chưa phát triển',

  // Token
  'The AZChain Platform ecosystem includes various types of tokens serving different purposes within the Blockchain platform and its applications. Below is the main Token:':
    'Hệ sinh thái AZChain Platform có nhiều loại Token phục vụ các mục đích khác nhau trong nền tảng và ứng dụng Blockchain.Dưới đây là Token chính:',
  'The native token of AZChain Platform, used to reduce transaction fees, participate inand utilize DeFi protocols.':
    'Token gốc của AZChain Platform, giúp giảm phí giao dịch, tham gia và sử dụng trong các giao thức DeFi.',
  'Purpose:': 'Mục địch:',
  'Functions:': 'Chức năng:',
  'Covers transaction fees, staking, gas payments, and participation in DeFi and NFT services on the AZChain Platform.':
    'Phí giao dịch, staking, thanh toán phí gas, và tham gia dịch vụ DeFi, NFT trên AZChain Platform.',
  'Smart Contract is audited by leading global security firms including Cirtik, Cyberscope, Quantstamp, and Salus prior to launch.':
    'Hợp đồng thông minh được kiểm toán bởi các công ty bảo mật hàng đầu thế giới như Cirtik, Cyberscope, Quantstamp, Salus trước khi phát hành.',

  // Roadmap
  'Team building': 'Xây dựng Đội ngũ',
  'Ecosystem development': 'Xây dựng các Nền tảng',
  'Community & Partner engagement': 'Kết nối Cộng đồng & Đối tác',
  'Q1–Q3: Develop AZC Pay Web3 Wallet, NFT Ecosystem & AZCARD':
    'Quý I - III: Phát triển\nVí Web3 AZG-Pay, NFT\nCổ đông & AZCARD',
  'Q4: Launch Real World Asset & NFT Marketplace & GameFi':
    'Quý IV: Ra mắt Real World Asset & NFT Marketplace & GameFi',
  'Q1: Public Token Sale, Launch Decentralized E-commerce Platform':
    'Quý I: Bán Public Token,\nra mắt nền tảng TMĐT\nPhi tập trung',
  'Q2–Q3: Expand AZC Coffee across Asia':
    'Quý II - III: Phát triển &\nmở rộng chuỗi AZC Coffee Toàn quốc và Đông Nam Á',
  'Q4: Token Listing': 'Quý IV: Niêm yết Token\n(Token Listing)',
  'Expand and develop the ecosystem (AI Insurance Hub, Carbon Exchange, ...)':
    'Mở rộng và phát triển Hệ sinh thái (AI Insurance Hub, Carbon Exchange,...)',
  'Full-scale development and expansion': 'Phát triển và Mở rộng\ntoàn diện',
  'IPO in Vietnam and Singapore': 'IPO tại Việt Nam và Singapore',

  Partners: 'Đối tác',
  'SMART CONTRACT': 'HỢP ĐỒNG THÔNG MINH',
  'International and will be implemented worldwide':
    'Mang tính quốc tế và sẽ được triển khai trên toàn thế giới',
  'Disclaimer: Cryptocurrency may be unregulated in your jurisdiction. The value of cryptocurrencies may go down as well as up. Profits may be subject to capital gains or other taxes applicable in your jurisdiction.':
    'Khuyến cáo: tiền mã hóa có thể không được quản lý trong khu vực của bạn. Giá trị của tiền mã hóa có thể giảm cũng như tăng. Lợi nhuận có thể chịu thuế lãi vốn hoặc các loại thuế khác áp dụng tại khu vực của bạn.',
  'Buy now': 'Mua ngay',
  'Terms of service': 'Điều khoản dịch vụ',
  'Privacy policy': 'Chính sách bảo mật',
  Disclaimer: 'Tuyên bố miễn trừ trách nhiệm',
  'Support center': 'Trung tâm hỗ trợ',

  // white home
  'Join the Blockchain Revolution with':
    'Tham gia cuộc cách mạng Blockchain cùng',
  'AZChain Platform Collection': 'Bộ sưu tập AZChain Platform',
  'Discover AZChain Platform with ease.':
    'Khám phá AZChain Platform một cách dễ dàng.',
  'The gateway to innovation, sustainable investment, and immersive Web3 experiences.':
    'Nơi hội tụ của Đổi Mới, Đầu Tư Bền Vững và Trải Nghiệm Web3 thú vị.',
  'AZCARD - Your gateway to Digital Asset ownership, fully integrated with AZGPAY for a complete Web3 experience.':
    'Thẻ Tài Sản Số - Định danh Blockchain Tiên phong cho cá nhân, tích hợp trực tiếp với Ví Web3 AZGPAY.',
  'ShareHolder NFTs': 'NFT Cổ đông',
  'Ownership, Investment, and Long-term Partnership with AZChain Platform.':
    'Quyền sở hữu, đầu tư và Hợp tác dài hạn với AZChain Platform.',
  'Web3 Payment Gateway & Global Coffee NFTs Investment Hub.':
    'Ví Web3 Thanh toán & Cổ đông NFT Coffee Toàn cầu.',
  Applications: 'Ứng dụng',
  'Over 11 integrated applications within the AZChain Platform ecosystem.':
    '11+ Ứng dụng tích hợp trong Hệ sinh thái AZChain Platform.',
  'AZCARD - Digital Asset Card': 'AZCARD - Thẻ Tài Sản Số',
  'AZCARD aims to become the standard Digital Asset Card for individuals and businesses in the Web3 era - empowering users to own Digital Assets and invest globally.':
    'AZCARD hướng tới trở thành Thẻ Tài Sản Số chuẩn mực dành cho Cá nhân và Doanh nghiệp trong Kỷ nguyên Web3, làm chủ Tài Sản Số và đầu tư Toàn Cầu.',
  'Ownership and Grow alongside the AZChain Platform.':
    'Quyền sở hữu và Phát triển cùng AZChain Platform.',
  'Unlock sustainable investment opportunities within the Web3 Ecosystem.':
    'Mở ra cơ hội đầu tư bền vững trong Hệ sinh thái Web3.',
  'AZGPAY Payment applies Blockchain Technology to the coffee industry.':
    'Thanh toán AZGPAY ứng dụng Blockchain trong ngành cà phê.',
  'Combining Global NFTs Coffee investment with transparency and innovation.':
    'Kết hợp đầu tư NFT Coffee Toàn Cầu minh bạch và hiện đại.',
  'E-commerce Products': 'Sản phẩm Thương Mại Điện Tử',
  'AZChain Platform unlocks The Multi-Industry Blockchain Ecosystem, delivering real value to customers, investors, and the global community':
    'AZChain Platform mở ra Hệ sinh thái Blockchain Đa ngành, mang đến giá trị thiết thực cho khách hàng, nhà đầu tư và cộng đồng toàn cầu'
}

export default translation
